...
const SAMPLE_ROWS: Row[] = [
  { Status: 'Proposal sent', Owner: 'Asha', 'Next step': '', 'Last activity': '2026-09-04' },
  { Status: 'Follow-up due', Owner: '', 'Next step': 'Call on Friday', 'Last activity': '2026-09-05' },
  { Status: 'Won', Owner: 'Ravi', 'Next step': 'Start handover', 'Last activity': '2026-09-08' },
  { Status: 'Open enquiry', Owner: '', 'Next step': '', 'Last activity': '2026-09-03' },
];
...
function DataLab() {
 ...
 const loadRows=(parsed:Row[], name:string)=>{const nextHeaders=Object.keys(parsed[0]||{});setRows(parsed);setFileName(name);setError('');setMapping({stage:matchHeader(nextHeaders,['stage','status']),last:matchHeader(nextHeaders,['last','date','activity']),owner:matchHeader(nextHeaders,['owner','assignee']),next:matchHeader(nextHeaders,['next','follow'])});};
 ...
 <section className={styles.dataGrid}><section className={styles.uploader}><label className={styles.drop}><input type="file" accept=".csv,text/csv" onChange={onFile}/><strong>{fileName || 'Choose a CSV file'}</strong><span>{fileName ? `${rows.length} records ready to inspect` : 'Enquiry, lead, CRM or spreadsheet export'}</span></label><button className={styles.sample} type="button" onClick={() => loadRows(SAMPLE_ROWS,'Vouch sample enquiries')}>Try sample data</button>{error && <p className={styles.error}>{error}</p>}
