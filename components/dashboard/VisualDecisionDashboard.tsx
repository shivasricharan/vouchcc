...
export default function VisualDecisionDashboard(){
  ...
  return <div className={styles.wrap}>
    <section className={styles.topline}>
      <div><span className={styles.eyebrow}>{dataMode==='demo'?'Sample decision view':'Your uploaded business data'}</span><h1>See the business in one glance.</h1><p>Visual signals show where momentum is building, where attention is leaking and what decision should happen next.</p></div>
    ...
    <section className={styles.kpis}>
      <article><span>Pipeline health</span><strong>{visuals.health}</strong><small>/100</small><div className={styles.meter}><i style={{width:`${visuals.health}%`}}/></div></article>
      ...
      <article><span>Value exposed</span><strong>{money(stats.atRiskValue)}</strong><small>{visuals.riskShare}% of open value</small></article>
      <article><span>Conversion signal</span><strong>{visuals.conversion}%</strong><small>{stats.wonCount} won · {stats.lostCount} lost</small></article>
    </section>

    <section className={styles.heroGrid}>
      <article className={`${styles.panel} ${styles.momentum}`}>
        <header><div><span>Momentum over time</span><h2>Is the pipeline moving?</h2></div><TrendingUp size={18}/></header>
      ...
      <article className={`${styles.panel} ${styles.priority}`}>
        <header><div><span>Decision now</span><h2>Highest-priority move</h2></div><Target size={18}/></header>
        <div className={styles.priorityVisual}><div className={styles.rings}><i/><i/><i/><b>{stats.stuckCount}</b><span>stalled</span></div><div><strong>{visuals.top?.title||'Review the oldest inactive opportunities'}</strong><p>{visuals.top?.businessImpact||`${stats.followUpCount} records need a clear owner and next action.`}</p></div></div>
        <div className={styles.priorityStrip}><span>Potentially affected</span><b>{money(stats.atRiskValue)}</b></div>
        <button onClick={()=>setPilot(true)}>Unlock guided action plan <ArrowRight size={15}/></button>
      </article>
    </section>

    <section className={styles.visualGrid}>
      <article className={styles.panel}><header><div><span>Pipeline distribution</span><h2>Where opportunities are sitting</h2></div><BarChart3 size={18}/></header><div className={styles.chartMedium}><ResponsiveContainer width="100%" height="100%"><BarChart data={visuals.stageData} layout="vertical" margin={{left:4,right:8}}><XAxis type="number" hide/><YAxis dataKey="name" type="category" width={92} tick={{fill:'#8da0b8',fontSize:10}} axisLine={false} tickLine={false}/><Tooltip cursor={{fill:'rgba(79,140,255,.05)'}} contentStyle={{background:'#091421',border:'1px solid rgba(137,167,207,.18)',borderRadius:10,fontSize:11}}/><Bar dataKey="count" radius={[0,7,7,0]}>{visuals.stageData.map((_,index)=><Cell key={index} fill={PALETTE[index%PALETTE.length]}/>)}</Bar></BarChart></ResponsiveContainer></div></article>

      <article className={styles.panel}><header><div><span>Attention heatmap</span><h2>How long records remain untouched</h2></div><Eye size={18}/></header><div className={styles.heatmap}>{visuals.heat.map((item,index)=>{const intensity=item.value/visuals.maxHeat;return <div key={item.label}><span>{item.label}</span><i style={{opacity:.18+intensity*.82,transform:`scaleY(${.45+intensity*.55})`}}/><b>{item.value}</b><small>{index<2?'Healthy':'Needs review'}</small></div>})}</div><p className={styles.microcopy}>Darker columns indicate a larger concentration of unattended opportunities.</p></article>

      <article className={styles.panel}><header><div><span>Lead source mix</span><h2>Where the pipeline originates</h2></div><Sparkles size={18}/></header><div className={styles.sourceSplit}><div className={styles.donut}><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={visuals.sourceData} dataKey="value" nameKey="name" innerRadius={48} outerRadius={74} paddingAngle={3}>{visuals.sourceData.map((_,index)=><Cell key={index} fill={PALETTE[index%PALETTE.length]}/>)}</Pie><Tooltip contentStyle={{background:'#091421',border:'1px solid rgba(137,167,207,.18)',borderRadius:10,fontSize:11}}/></PieChart></ResponsiveContainer><b>{stats.total}</b><span>records</span></div><div className={styles.legend}>{visuals.sourceData.map((item,index)=><div key={item.name}><i style={{background:PALETTE[index%PALETTE.length]}}/><span>{item.name}</span><b>{item.value}</b></div>)}</div></div></article>
    </section>

    <section className={styles.previewGate}>
      <div className={styles.lock}><LockKeyhole size={20}/></div><div><span>Preview complete</span><h2>The demo shows the signals. The paid pilot turns them into a working decision system.</h2><p>Detailed root-cause analysis, record-level recommendations, owner assignment, role-specific views and outcome tracking are reserved for the guided 14-day pilot.</p></div><div className={styles.gateActions}><button className={styles.secondary} onClick={()=>setEmail(true)}><Mail size={15}/> Email visual brief</button><button className={styles.primary} onClick={()=>setPilot(true)}>Explore ₹9,999 pilot <ArrowRight size={15}/></button>{dataMode==='demo'&&<button className={styles.upload} onClick={()=>setShowUpload(true)}><Upload size={15}/> Use my CSV</button>}</div>
    </section>
