...
export default function DashHeader(){
  ...
  return <header className="demo-header">
    <a href="https://yourvouch.com" className="demo-brand" aria-label="Vouch website"><b>Vouch</b></a>
    ...
    <div className="demo-header-actions">
      <a className="header-quiet hidden lg:inline-flex items-center" href="https://vouchstarterkit.netlify.app/" target="_blank" rel="noopener noreferrer">Open-source foundation</a>
      {dataMode==='demo'&&view==='dashboard'&&<button className="header-quiet" onClick={refreshAnalysis}><RotateCcw size={14}/><span>Replay</span></button>}
      <button className="header-upload" onClick={()=>setShowUpload(true)}><Upload size={15}/><span>Upload new data</span></button>
      <button className="header-menu" onClick={()=>setShowGuide(true)} aria-label="Open demo guide"><MoreHorizontal size={18}/></button>
    </div>
  </header>;
}
